<?php
/*
Plugin Name: FORS MailWizz Newsletter Box
Plugin URI: http://www.mailwizz.com
Description: Adds a subscription widget for your <a href="http://www.mailwizz.com/" target="_blank">MailWizz Email Marketing Application</a> based on the <a href="https://github.com/twisted1919/mailwizz-php-sdk" target="_blank">PHP-SDK</a>. <br />Using the widget, you can generate a subscription form based on your mail list definition. You also have full control over the generated form, so you can continue changing it until it fits your needs. Customized for FORS needs
Version: 1.4
Author: Serban Cristian <cristian.serban@mailwizz.com>, Jonas Boiziau <jonas.boiziau@fors.unil.ch>
    ...
    protected function generateForm(array $instance)
    {
        ...
        foreach ($freshFields as $field) {
			if($field['type']['identifier'] == 'dropdown'){
				...
				$output[] = $ddOutput;
			}elseif($field['type']['identifier'] == 'checkbox'){
				$required = $field['required'] != 'yes' ? '' : 'required';
				$required_span = $field['required'] != 'yes' ? '' : '<span class="required">*</span>';
				
				$ddOutput = '<div class="form-group checkbox"><label>'.$field['label'].' '.$required_span.'</label>';
				
				$ddOutput .= '<input type="checkbox" class="form-control" name="'.$field['tag'].'" placeholder="'.$field['help_text'].'" '.$required.'>';
